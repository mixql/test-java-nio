...
object MainServerApp {
  def main(args: Array[String]): Unit = {
    ...
    println("host of server is " + host + " and port is " + port + " and base path is " +
      basePath.getAbsolutePath
    )
    val module3 = ClientModule("module-scala3", host, port, basePath)
    try {
      import app.zio.grpc.remote.clientMsgs.*
      module3.sendMsg(ZioMsgTest1("hello", "scala", "3")) match {
        case ZioMsgTestReply(msg, _) => println(s"Got response from client module-scala-3: " + msg)
      ...
      case e: Throwable => println("Got exception: "+ e.getMessage)
    } finally {
      module3.close()
    }
  }
